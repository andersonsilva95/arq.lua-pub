...
{	
    ...
    {
        ID      = "ATTACK01",
        Motion  = "goblin_torchman_attack01.frm",
    ...
    {
        ID      = "ATTACK02",
        Motion  = "goblin_torchman_attack02.frm",
...
CallBack =
{
    Land    = "OnGoblin_torchman_Land",
    Die     = "OnGoblin_torchman_Die",
    Damage  = "OnGoblin_torchman_Damage",
}
...
function InitGoblin_torchman( iMonsterIndex )
...
function Stone_clan_goblin_Attack01Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 57 ) then
		if( pMonster:GetIsRight() ) then
			pMonster:AddDamage(DT_FIRE_HAMMER_TORCHMAN_FIRE)
		else
			pMonster:AddDamage(DT_FIRE_HAMMER_TORCHMAN_FIRE_R)
		end
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
...
function Stone_clan_goblin_Attack02Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 62 ) then
		if( pMonster:GetIsRight() ) then
			pMonster:AddDamage(DT_FIRE_HAMMER_TORCHMAN_SPYBOOM)
		else
			pMonster:AddDamage(DT_FIRE_HAMMER_TORCHMAN_SPYBOOM_R)
		end
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
...
function Stone_clan_goblin_Check_Walk( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 450, -180, 300, 180, -250 ) == true and pMonster:FloatRand() < 0.75) then
			pMonster:TurnToTarget()
			pMonster:SetState( "WALK" )
			pMonster:ResetDelay()
	end
...
function Stone_clan_goblin_Check_Stop( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 50, -250, 300, 250, -250 ) == true ) then
...
function Stone_clan_goblin_Check_Attack02( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 0, -300, 250, 300, -200 ) == true and pMonster:FloatRand() < 0.75 ) then
...
-- 콜백 함수
-----------------------------------------------------------------------
function OnGoblin_torchman_Land( iMonsterIndex )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
function OnGoblin_torchman_Die( iMonsterIndex )
	local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
function OnGoblin_torchman_Damage( iMonsterIndex )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
