...
{	
    -- 대기
    {
        ...
        Trigger =
        {
            ...
            {
                AICheckFrame        = 165,
                AIFunc              = "Goblin_berserker_Check_Attack04",
                DelayFrame          = 220,
            },
        },
    },
    ...
    {
        ID      = "ATTACK04",
        Motion  = "goblin_berserker_attack04.frm",
        Func    = "Goblin_berserker_Attack04Func",
    },
...
function Goblin_berserker_Attack01Func( iMonsterIndex, iFrame )
    ...
    elseif( iFrame == 60 ) then
		pMonster:AddDamage(DT_BRUTUS_HAMMER_SMASH)
	...
	elseif( iFrame == 68 ) then
		...
		pMonster:AddParticle( "Scolpis_Rush_02", 0.1, -0.05 )
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
...
function Goblin_berserker_Attack02Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 50 ) then
		pMonster:AddDamage(DT_BRUTUS_HAMMER_SMASH_SPIN)
...
--
function Goblin_berserker_Attack04Func( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
	...
	elseif( iFrame == 67 ) then
		...
		if( pMonster:GetIsRight() ) then
			pMonster:AddDamageWithLocateAngle( DT_BRUTUS_HAMMER_THROW_ATK, 0.11, 0.18, false, angle )
		else
			pMonster:AddDamageWithLocateAngle( DT_BRUTUS_HAMMER_THROW_ATK, 0.11, 0.18, false, -angle )
		end
...
function Goblin_berserker_Check_Walk( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 500, -220, 360, 220, -400 ) == true ) then
			pMonster:TurnToTarget()
			pMonster:SetState( "WALK" )
			pMonster:ResetDelay()
	end
...
function Goblin_berserker_Check_Stop( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 50, -250, 180, 250, -200 ) == true or pMonster:CheckTargetInRange( 650, -200, 350, 200, -300 ) == false) then
...
function Goblin_berserker_Check_Attack03( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 300, -400, 300, 400, -300 ) == true and pMonster:FloatRand() < 0.75 ) then
		pMonster:SetState( "ATTACK03" )
		pMonster:ResetDelay()
	end
end

function Goblin_berserker_Check_Attack04( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )    
...
function OnGoblin_berserker_Damage( iMonsterIndex )
	...
	if( pMonster:CheckState( "DOWN_UP01" ) == false and pMonster:CheckState( "DOWN_UP02" ) == false and pMonster:CheckState( "DOWN_MIDDLE" ) == false and pMonster:CheckState( "DOWN_DOWN" ) == false and pMonster:CheckState( "DOWN_SPECIAL" ) == false) then
		if( pMonster:FloatRand() < 0.05 and pMonster:GetIsContact() == true and pMonster:IsSuperArmor() == false ) then
			pMonster:TurnToTarget()
			if( pMonster:CheckTargetInRange( 0, -400, 50, 400, -100 ) == true ) then
				pMonster:SetState( "ATTACK02" )
			else
				pMonster:SetState( "ATTACK03" )
			end
