...
{	
    ...
    {
        ID      = "DIE_LANDING",
        Motion  = "soldier_guard_machine_die.frm",
...
function Spider_guard_Machine_Attack01Func( iMonsterIndex, iFrame )
    ...
    elseif( iFrame == 60 ) then
		pMonster:AddDamage(DT_SOLDIER_GUARD_DEMOLITION_LAY)
...
function Spider_guard_Machine_Attack02Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 79 ) then
		pMonster:AddDamage( DT_SOLDIER_GUARD_BOMB	 )
		--onster:AddDamage( DT_SOLDIER_GUARD_BOMB1	 )
		pMonster:AddDamage( DT_SOLDIER_GUARD_BOMB_R	 )
		--onster:AddDamage( DT_SOLDIER_GUARD_BOMB1_R )
...
function Spider_guard_Machine_Attack03Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 45 ) then
		...
		pMonster:AddDamageWithLocate( DT_SOLDIER_GUARD_AIM_MARK, pMonster:GetTargetX(), pMonster:GetTargetY(), true )
	elseif( iFrame == 90 ) then
		local posx = 0.15
		if( pMonster:GetIsRight() ) then
			pMonster:AddTraceParticleToBone( "Spider_Machine_Vulcan_Shot_01", 1.0, 32, 6, 6, posx + 0.04, 0.0 )
			pMonster:AddTraceParticleToBone( "Spider_Machine_Vulcan_Shot_02", 1.0, 32, 6, 6, posx + 0.04, -0.01 )
		else
			pMonster:AddParticleNoDirection( "Spider_Machine_Vulcan_Shot_01_R", -0.19, 0.29 )
			pMonster:AddTraceParticleToBone( "Spider_Machine_Vulcan_Shot_02", 1.0, 32, 8, 6, -posx - 0.03, -0.01 )
			pMonster:AddParticleNoDirection( "Spider_Machine_Vulcan_Shot_01_R", -0.03, 0.28 )
			pMonster:AddTraceParticleToBone( "Spider_Machine_Vulcan_Shot_02", 1.0, 33, 8, 6, -posx + 0.01, 0.0 )
		end
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
...
function OnSpider_guard_Machine_Damage( iMonsterIndex )
	...
	if( pMonster:CheckState( "DOWN_MIDDLE" ) == false and pMonster:CheckState( "DOWN_DOWN" ) == false and pMonster:CheckState( "DOWN_SPECIAL" ) == false and pMonster:CheckState( "STONECURSE" ) == false and pMonster:CheckState( "ICECURSE" ) == false ) then
		if ( pMonster:FloatRand() < 0.05 and pMonster:GetIsContact() == true ) then
			pMonster:SetState( "ATTACK03" )
		end
	end
...
function Spider_guard_Machine_Smoke( iMonsterIndex, iFrame )
	...
	if( math.mod( iFrame, 100 ) == 1 ) then
		...
		if( pMonster:GetIsRight() ) then
			pMonster:AddTraceParticleToBone( "Talin_Steam_03", 1.0, 9, 8, 8, posx, posy )
			pMonster:AddTraceParticleToBone( "Talin_Steam_03", 1.0, 11, 8, 8, posx -0.05, posy )
		else
			posx = posx * -1.0
			pMonster:AddTraceParticleToBone( "Talin_Steam_03_R", 1.0, 9, 8, 8, posx, posy, true )
			pMonster:AddTraceParticleToBone( "Talin_Steam_03_R", 1.0, 11, 8, 8, posx +0.05, posy, true )
		end
    end
end
