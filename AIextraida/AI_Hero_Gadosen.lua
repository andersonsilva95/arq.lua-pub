...
ActionList =
{
    ...
    {
        ID      = "ATTACK07",
        Motion  = "gagosen_hero_attack02.frm",
        Func    = "Gadosen_Attack07Func",
    },
    
    -- 지옥섬광
    {
        ID      = "ATTACK08",
        Motion  = "gagosen_hero_attack03.frm",
        Func    = "Gadosen_Attack08Func",
    },
...
function Gadosen_Attack04Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 51 ) then
        ...
        pMonster:AddDamage( DT_HERO_GADOSEN_ATK4 )
        pMonster:PlaySound(987028)
...
function Gadosen_Attack06Func( iMonsterIndex, iFrame )
	...
	elseif( iFrame == 16 ) then
		pMonster:AddDamageWithLocate( DT_HERO_GADOSEN_METEO_STARTER,0.25, 0.95,true )
	elseif( iFrame == 26 ) then
	    pMonster:AddDamageWithLocate( DT_HERO_GADOSEN_METEO_STARTER,1.45, 1.59,true )
	elseif( iFrame == 36 ) then
	    pMonster:AddDamageWithLocate( DT_HERO_GADOSEN_METEO_STARTER,3.0, 0.62,true )
	elseif( iFrame == 46 ) then
	    pMonster:AddDamageWithLocate( DT_HERO_GADOSEN_METEO_STARTER,3.9, 1.2,true )
    elseif( iFrame >= pMonster:GetLastFrame() ) then
		pMonster:SetState( "WAIT" )
    end
end
...
--  헬게이트
function Gadosen_Attack07Func( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
	...
	elseif( iFrame == 20 ) then
		pMonster:CameraVib( 0, 1.2, 0.008 )
		if( pMonster:GetIsRight() ) then
			pMonster:AddDamage(DT_HERO_GADOSEN_BLACKHOLE)
		else
			pMonster:AddDamage(DT_HERO_GADOSEN_BLACKHOLE_R)
		end
		
		
		pMonster:PlaySound(986013)
		pMonster:PlaySound(986014)
	elseif( iFrame >= 21 and iFrame <= 145 ) then
...
--  지옥섬광
function Gadosen_Attack08Func( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )

    if( iFrame <= 1 ) then
		pMonster:SetSuperArmor( true )
	elseif( iFrame == 58 ) then
		pMonster:AddDamage(DT_HERO_GADOSEN_LIGHTOFHELL)
		pMonster:PlaySound(986015)
    elseif( iFrame >= pMonster:GetLastFrame() ) then
		pMonster:SetState( "WAIT" )
    end
end
...
function Gadosen_Check_Attack07( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
function Gadosen_Check_Attack08( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
