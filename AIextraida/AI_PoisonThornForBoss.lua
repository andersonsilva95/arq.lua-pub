...
{	
    -- 대기
    {
        ...
        Trigger =
        {
            ...
            {
                AICheckFrame        = 120,
                AIFunc              = "Spinemon_Check_Suicidal",
                DelayFrame          = 20,
            },
        },
    },
...
-- 죽는상태
function Spinemon_SuicidalFunc( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
-- SUMMON!
function Spinemon_SummonFunc( iMonsterIndex, iFrame )
	local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
	
	if( iFrame == 1 ) then
		pMonster:AddParticle("Divine_Tree_Summon_01", 0.0, 0.0)
		pMonster:AddParticle("Divine_Tree_Summon_02", 0.0, 0.0)
		pMonster:AddParticle("Divine_Tree_Summon_03", 0.0, 0.0)
	elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
end
...
function Spinemon_Check_Targeting( iMonsterIndex, iFrame )
    ...
    if( pMonster:SetTarget( 0, -1100, 600, 1100, -400 ) == true ) then
        pMonster:ResetDelay()
    end
end
...
function Spinemon_Check_ReleaseTarget( iMonsterIndex, iFrame )
	...
	if( pMonster:CheckTargetInRange( 200, -700, 700, 700, -500 ) == false ) then
...
function Spinemon_Check_Walk( iMonsterIndex, iFrame )
    ...
    if( pMonster:CheckTargetInRange( 600, -400, 500, 400, -300 ) == true ) then
		pMonster:TurnToTarget()
        pMonster:SetState( "WALK" )
        pMonster:ResetDelay()
    end
...
function Spinemon_Check_HighJump( iMonsterIndex, iFrame )
    ...
    if( pMonster:CheckTargetInRange( 300, -350, 550, 350, 100 ) == true and pMonster:GetIsContact() == true and pMonster:GetTargetIsContact() == true ) then
...
function Spinemon_Check_JumpDown( iMonsterIndex, iFrame )
   ...
   if( pMonster:CheckTargetInRange( 300, -300, -140, 300, -400 ) == true ) then
        pMonster:AddCountValue(1)
...
function Spinemon_Check_Attack04( iMonsterIndex, iFrame )
    ...
    if( pMonster:CheckTargetInRange( 400, -300, 500, 300, -100 ) == true ) then
		pMonster:AddCountValue(1)
...
function Spinemon_Check_Suicidal( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )

    if( pMonster:GetTimer() >= 55 * 30 ) then
		pMonster:TurnToTarget()
        pMonster:SetState( "SUICIDAL" )
        pMonster:ResetDelay()
    end
end
-----------------------------------------------------------------------
