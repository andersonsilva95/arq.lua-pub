...
{	
    ...
    {
        ID      = "ATTACK03",
        Motion  = "GhostFish_Attack03.frm",
        Func    = "GhostFish_Attack03Func",
    },
    ...
    -- 다운 상
    {
        ...
        Trigger =
        {
            {
                AICheckFrame        = 1,
                AIFunc              = "GhostFish_CheckDownUp02",
                DelayFrame          = 0,
            },
        }
    },
    ...
    {
        ID      = "DOWN_DOWN",
        Motion  = "ghostfish_down_down.frm",
...
function GhostFish_Attack02Func( iMonsterIndex, iFrame )
	...
	if( iFrame == 40 ) then
		pMonster:PlaySound(1408)
	elseif( iFrame == 58 ) then
		pMonster:AddDamageWithLocate( DT_FIREBALL_DARK, 0.26, 0.00 )
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        pMonster:SetState( "WAIT" )
    end
end
