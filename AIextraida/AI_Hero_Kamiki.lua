...
{	
    ...
    -- 블리딩
    {
        ID      = "ATTACK10",
        Motion  = "Kamiki_Attack05.frm",
        Func    = "Kamiki_Attack10Func",
    },

    -- 점프
    {
        ID      = "JUMP",
        Motion  = "Kamiki_Jump_up.frm",
    ...
    {
        ID      = "JUMP_DOWN",
        Motion  = "Kamiki_Jump_down.frm",
        Func    = "Kamiki_JumpDownFunc",
    },
    
    -- 점프
    {
        ID      = "JUMP_LANDING",
        Motion  = "Kamiki_Jump_landing.frm",
        Func    = "Kamiki_ToWait",
    },
...
function Kamiki_Attack06Func( iMonsterIndex, iFrame )
    ...
    elseif( iFrame == 68 ) then
        ...
        --pMonster:AddDamage( DT_HERO_KAMIKI_ATK2 )
        local angle = 0.5
        ...
        pMonster:AddDamageWithLocateAngle( DT_HERO_KAMIKI_ATK2, 0.0, 0.0, false, angle )
        ...
        pMonster:AddDamageWithLocateAngle( DT_HERO_KAMIKI_ATK2_R, 0.0, 0.0, false, angle )
...
function Kamiki_Attack07Func( iMonsterIndex, iFrame )
    ...
    elseif( iFrame == 81 ) then
		...
		pMonster:PlaySound(995027)
        pMonster:AddDamage( DT_HERO_KAMIKI_ICEFIELD )
...
function Kamiki_Attack08Func( iMonsterIndex, iFrame )
    ...
    elseif( iFrame == 70 ) then
        ...
        pMonster:KamikiLightning( pMonster:GetX() - 0.3, pMonster:GetY() )
		for i = 0, 8 do
	        pMonster:AddDamageWithLocateAngle( DT_HERO_KAMIKI_ATK3, 0.0, 0.2, false, -0.0 + i * 0.7 )
	    end
		pMonster:PlaySound(986011)
...
--  블리딩
function Kamiki_Attack10Func( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
    ...
    elseif( iFrame == 71 ) then
		...
		pMonster:PlaySound(995028)
    elseif( iFrame == 80 ) then
		pMonster:AddDamageToAlivePlayer(DT_HERO_KAMIKI_ATK1, 0.0, 0.2, true)
		pMonster:PlaySound(986010)
...
--  점프 다운 상태
function Kamiki_JumpDownFunc( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
--  점프 상태
function Kamiki_ToWait( iMonsterIndex, iFrame )
    local pMonster = g_MyD3D:GetMonster( iMonsterIndex )
...
function Kamiki_TeleportLandingFunc( iMonsterIndex, iFrame )
    ...
    elseif( iFrame >= pMonster:GetLastFrame() ) then
        ...
        elseif( pMonster:GetCountValue() == 2 ) then
            pMonster:SetState( "ATTACK10" )   -- 블리딩
		...
		elseif( pMonster:GetCountValue() == 6 ) then
...
function Kamiki_Check_Targeting( iMonsterIndex, iFrame )
    ...
    if( pMonster:SetTarget( 100, -800, 700, 800, -700 ) == true ) then
        pMonster:ResetDelay()
    end
end
...
function Kamiki_Check_JumpAttack( iMonsterIndex, iFrame )
    ...
    if( pMonster:CheckTargetInRange( 100, -300, 600, 300, 200 ) == true and pMonster:GetIsContact() == true and pMonster:GetTargetIsContact() == true ) then
...
function Kamiki_Check_Teleport( iMonsterIndex, iFrame )
    ...
    if( pMonster:IsTarget() == true and pMonster:CheckTargetInRange( 200, -600, 250, 600, -250 ) == false ) then
        ...
        local posy = pMonster:GetTargetY() + 0.1
		if( posy < 0.0 ) then
			posy = 0.1
		end
		pMonster:SetY( posy )
